...
class DatabaseCpiPersistence(private val entityManagerFactory: EntityManagerFactory) : CpiPersistence {
    ...
    override fun store(
        ...
        return entityManagerFactory.createEntityManager().transaction { em ->
            ...
            // Sometimes, we only want to allow creation of new CpiMetadataEntity records, not updating of existing ones
            ...
            if (cpiMetadataInDb != null && !allowCpiUpdate) {
                // If we already have this ID (the synthetic key of cpiMedataKey) and updates are forbidden then
                // fail.
            ...
            val cpiCpkEntities = cpi.cpks.mapTo(HashSet()) { cpk ->
                ...
                // CPI parent and that violates a foreign key constraint.
                CpiCpkEntity(
                    cpiCpkKey,
