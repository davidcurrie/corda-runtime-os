...
class MessagingFlow : RPCStartableFlow {
    ...
    override fun call(requestBody: RPCRequestData): String {
        ...
        val received = session.sendAndReceive(MyClass::class.java, MyClass("Serialize me please", 1)).unwrap { it }
        ...
        val received3 = session.receive(MyClass::class.java).unwrap { it }
...
class MessagingInitiatedFlow : ResponderFlow {
    ...
    override fun call(session: FlowSession) {
        ...
        val received = session.receive(MyClass::class.java).unwrap { it }

        log.info("Received data from peer: $received")

        session.send(received.copy(string = "this is a new object", int = 2))

        val received2 = session.receive(MyClass::class.java).unwrap { it }
